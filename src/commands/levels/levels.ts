...
export default class LeaderboardCommand extends Command {
  ...
  public async main({ msg, args }: RunArgumentsOptions) {
    let page = 0;
    if (args[0]) {
      const tmp = parseInt(args[0]) - 1;
      if (!isNaN(tmp)) page = Math.max(0, tmp);
    }

    const leaderboard = await this.client.database.levels.fetchLeaderboard(page);
    if (!leaderboard.length) return msg.channel.send(`:x: No data for page ${page + 1}.`);
    ...
    const lengthNameArray = await Promise.all(
      leaderboard.map(async (u) => {
        const usr = await this.client.users.fetch(u.id).catch((_) => null);
        return usr.username.length;
      })
    );
    const lengthName = Math.max(...lengthNameArray);
    const lengthXp = Math.max(...leaderboard.map((u) => this.client.util.formatNumber(leaderboard[0].totalXp).length));
