...
export class ClientUtil {
  ...
  public async getUser(message: Message, arg: string): Promise<User> {
    if (message.mentions.users.size > 0) { return (message.mentions.users.first() as User) }
    if (arg && /[0-9]{16,18}/.test(arg)) { return (this.client.users.fetch(arg) as User | Promise<User>) }
    if (arg && /[a-zA-Z]{1,30}/) {
